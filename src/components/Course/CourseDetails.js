...
export default function CourseDetails() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost/Backend/api/submit_application.php', formData, {
        headers: {
      ...
      setFormMessage('Error submitting the application.');
  ...
  return (
    <div>
      ...
      <div className=" mt-3">
        <center> <button className="btn btn-danger" onClick={handleApplyNowClick}>
          Apply Now
        </button></center>
      </div>
